...
(() => {
  ...
  function setDirection(candidate) {
    ...
    if (!candidate || (lastDirection.x + candidate.x === 0 && lastDirection.y + candidate.y === 0)) return;
    if (lastDirection.x === candidate.x && lastDirection.y === candidate.y) return;
    if (directionQueue.length < 2) directionQueue.push(candidate);
  }
